...
public class ExamHelp {
     ...
     **/
    public static void postWithJson(String paperId, String token, String cookie, String userAgent) throws Exception {
            ...
            System.err.println("====================================欢迎你，"+data.get("userId_dictText"));
            System.err.println("====================================你正在参加考试：" +data.get("departId_dictText"));
            //单选题
            getAnswer("radioList", paperId, token, cookie, userAgent, 4,"3");

            //判断题
            getAnswer("judgeList", paperId, token, cookie, userAgent, 2,"3");

            //多选题
            getAnswer("multiList", paperId, token, cookie, userAgent, 11,"1");

            //主观题
            getAnswer( "saqList", paperId, token, cookie, userAgent, 1,"2");
    ...
    public static void getAnswer(String listName, String paperId, String token,
                                 String cookie, String userAgent, int num, String flag) throws Exception {
        for (int j = 0; j < num; j++) {
            JSONObject request = new JSONObject();
            request.put("id", paperId);
            JSONObject data1 = post(request, "http://221.11.103.120:8101/exam/api/paper/paper/paper-detail", paperId, token, cookie, userAgent);
            JsonArray jsonArray = new JsonParser().parse(data1.get(listName).toString()).getAsJsonArray();
            for (int i = 0; i < jsonArray.size(); i++) {
                if (jsonArray.get(i).getAsJsonObject().get("isRight").toString().equals("false")) {
                    ...
                    System.err.println("====================================这是第"+questionNo+"题==============================");
                    getAnswerDetail(questionId, paperId, token, cookie, userAgent, j, flag);
                }
            }
        }
    }
    ...
    public static void getAnswerDetail(String quId, String paperId, String token,
        ...
        System.out.println("问题:" + quDetail.get("content"));
        ...
        if ("1".equals(flag)) {
            ...
            System.out.println("现在开始尝试选择"+change(String.valueOf(num))+answerList.get(num).getAsJsonObject().get("content"));
        }
        fillDetail(paperId, token, cookie, userAgent, quId, list,answers);
    }


    public static void fillDetail(String paperId, String token, String cookie, String userAgent, String quId,
                                  List<String> answers,String answer) throws Exception{
        JSONObject request = new JSONObject();
        request.put("paperId", paperId);
        ...
        System.out.println("作答结果:" + (Integer.valueOf(response.get("statusCode").toString())==200?"成功":"失败"));
//        Thread.sleep(500);
    }
    ...
    public static List getchoose(int num, List list, JsonArray answerList) {
        ...
        System.out.println("现在开始尝试选择选择"+change(aList.toString()));
